...
export class Filecontrol {
    ...
    insertdata = async (req: Request, res: Response) => {
        const filepath: string = path.join(__dirname, `../../uploads/myfile.csv`);
        let content: string = `\n${req.body.firstname},${req.body.lastname},${new Date().toISOString()}`;
        await this.fileservise.writing(filepath, content, { flag: 'a' });

        const data: string[][] = await this.fileservise.reading(filepath);
    ...
    deletedata = async (req: Request, res: Response) => {
        ...
        data.forEach(element => {
            if (element[0] != req.query.id) {
                const date: Date = new Date((element[2]) ? element[2] : 'undefined');
        ...
        const data2: string[][] = await this.fileservise.reading(filepath);

        res.render('display', { dataarray: data2, filename: req.query.filename });
    }

    fileread = async (req: Request, res: Response) => {
        const filepath: string = path.join(__dirname, `../../uploads/myfile.csv`);
        const data: string = await this.fileservise.reading(filepath);
        console.log(data);
    }
